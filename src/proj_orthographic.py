"""
    ...
    Orthographic projection for projecting DEM vertices to 3D coordinates.
...
class proj_orthographic:
    """
    Class for projecting DEM vertices to 3D coordinates using an orthographic projection.
    ...
    def project(self, v) -> tuple:
        ...
        lon = wmx2lon(v[0])
        lat = wmy2lat(v[1])
        x = PT_EARTH_RADIUS * math.cos(lat) * math.sin(lon - self.lonc)
        y = PT_EARTH_RADIUS * (self.c1 * math.sin(lat) - self.s1 * math.cos(lat) * math.cos(lon - self.lonc))
        return (x, y, v[2] + self.z_offset)
